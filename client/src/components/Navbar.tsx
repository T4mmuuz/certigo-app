  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2 font-bold text-2xl text-primary hover:opacity-90 transition-opacity">
        ...
        <form onSubmit={handleSearch} className="flex-1 max-w-md relative hidden md:block">
          ...
          <Input
            ...
            className="pl-9 bg-muted/50 border-transparent focus:bg-background transition-all rounded-full"
          />
        </form>
        ...
        <div className="flex items-center gap-2">
                ...
                <ScrollArea className="h-72">
                  {notifications.length === 0 ? (
                    ...
                    <DropdownMenuItem
                      ...
                      onClick={() => { if (!notif.readAt) markReadMutation.mutate(notif.id); }}
                    >
                      <div className="flex items-center gap-2 w-full">
                        <span className="font-medium text-foreground text-sm">{notif.title}</span>
                        {!notif.readAt && <span className="w-2 h-2 bg-primary rounded-full ml-auto flex-shrink-0" />}
                      </div>
          ...
          {user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="gap-2 rounded-full pl-2 pr-4 h-10 border border-border hover:bg-muted/50">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center text-primary-foreground font-bold text-xs">
                    {user.name?.charAt(0)?.toUpperCase() || user.username.slice(0, 1).toUpperCase()}
                  </div>
                  <span className="hidden sm:inline-block font-medium text-sm">{user.name || user.username}</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 rounded-xl p-2 bg-card border shadow-lg">
                ...
                <DropdownMenuItem onClick={() => setLocation("/chat")} className="cursor-pointer rounded-lg">
                  <MessageSquare className="w-4 h-4 mr-2" /> {t("nav.messages")}
                </DropdownMenuItem>
